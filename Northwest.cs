...
namespace Exam
{
    internal class Northwest
    {
        ...
        public void Decision(int[,] rate, int[] post, int[] potr, out int[,] answer)
        {
            ...
            answer = new int[post.Length, potr.Length]; // задание размерности массива
            while (post.Sum() != 0 && potr.Sum() != 0)
            {
                for (int i = 0; i < cloneRate.GetLength(0); i++)
                { 
                    for (int j = 0; j < cloneRate.GetLength(1); j++)
                    {
                        if (post[i] > potr[j])
                        {
                            answer[i, j] = potr[j];
